...
def displayIndex(testImg):
  ...
  normalizeFaceVector = np.zeros((143,51260), order='C')
  ...
  # resized_img = np.reshape(imageArray[70,:],(233,220))
  ...
  matrix_w = np.zeros((k,143))
  #------storing lower dimensionality k eigenvectors into matrix_w-----------
  for i in range(k):
    matrix_w[i] = np.array(eig_pairs[i][1].reshape(1,143))
